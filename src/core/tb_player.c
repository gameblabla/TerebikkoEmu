/* tb_player.c - threaded ffmpeg decode + SDL3 audio output. */
#include "tb_player.h"
...
static void lock(tb_player *p) { SDL_LockMutex(p->mtx); }
static void unlock(tb_player *p) { SDL_UnlockMutex(p->mtx); }
...
static double mono_sec(void) { return (double)SDL_GetTicksNS() / 1.0e9; }
...
static void advance_clock(tb_player *p) {
    ...
    if (audio_expected && p->total_put == 0) {
        ...
        int queued = (int)SDL_GetAudioStreamQueued(p->audio);
...
static int decode_thread(void *arg) {
    ...
    while (1) {
        ...
        int aqueued = p->audio ? (int)SDL_GetAudioStreamQueued(p->audio) : 0;
        ...
        if (seek >= 0) {
            ...
            lock(p);
            queue_clear(p);
            if (p->audio) SDL_ClearAudioStream(p->audio);
        ...
        if (!should_read) {
            ...
            SDL_Delay(5); continue;
        }

        int r = av_read_frame(p->fmt, pkt);
        if (r < 0) { if (DBG) fprintf(stderr, "[pl] read EOF/err=%d q=%d\n", r, p->qcount); lock(p); p->ended = true; unlock(p); SDL_Delay(15); continue; }
                ...
                while (avcodec_receive_frame(p->actx, frm) >= 0) {
                    ...
                    if (feed) {
                        ...
                        if (got > 0) {
                            ...
                            SDL_PutAudioStreamData(p->audio, out, bytes);
...
tb_player *tb_player_open(const char *path, tb_hwaccel hw, char *errbuf, size_t errsz) {
    if (!SDL_InitSubSystem(SDL_INIT_AUDIO)) { /* audio may be unavailable; continue clock-only */ }
    ...
    /* audio device opened at a fixed 48 kHz stereo float; SDL converts to the device's
     * real format, and swr (configured lazily from the first frame) converts the source
     * to this. */
    if (p->astream >= 0) {
        p->dev_ch = 2; p->dev_rate = 48000;
        SDL_AudioSpec spec; SDL_zero(spec); spec.format = SDL_AUDIO_F32; spec.channels = p->dev_ch; spec.freq = p->dev_rate;
        p->audio = SDL_OpenAudioDeviceStream(SDL_AUDIO_DEVICE_DEFAULT_PLAYBACK, &spec, NULL, NULL);
        if (p->audio) {
            SDL_ResumeAudioStreamDevice(p->audio);
            p->bytes_per_sec = (double)p->dev_rate * p->dev_ch * sizeof(float);
        }
    }

    p->mtx = SDL_CreateMutex();
    p->last_wall = mono_sec();
    p->thread = SDL_CreateThread(decode_thread, "tb_decode", p);
    return p;
}
...
void tb_player_close(tb_player *p) {
    ...
    if (p->thread) SDL_WaitThread(p->thread, NULL);
    if (p->audio) SDL_DestroyAudioStream(p->audio);
    if (p->swr) swr_free(&p->swr);
    ...
    av_free(p->disp);
    if (p->mtx) SDL_DestroyMutex(p->mtx);
    free(p);
}
...
void tb_player_play(tb_player *p) {
    ...
    if (p->audio) { SDL_ClearAudioStream(p->audio); SDL_ResumeAudioStreamDevice(p->audio); }
    unlock(p);
}
void tb_player_pause(tb_player *p) { lock(p); advance_clock(p); p->playing = false; if (p->audio) SDL_PauseAudioStreamDevice(p->audio); unlock(p); }
...
void tb_player_set_muted(tb_player *p, bool m) { lock(p); p->muted = m; if (p->audio) SDL_ClearAudioStream(p->audio); p->need_audio_start = true; unlock(p); }
